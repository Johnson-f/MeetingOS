...
impl TursoClient {
    pub async fn create_or_get_meeting(
        ...
        conn.execute(
            "INSERT INTO meetings (
                ...
                original_meeting_url, platform, dedup_key, scheduled_start_at, status,
            ...
            (
                ...
                draft.scheduled_start_at.as_deref(),
                created_at.as_str(),
                created_at.as_str(),
            ),
        )
    ...
    pub async fn list_meetings_for_user(
        ...
        while let Some(row) = rows.next().await? {
            meetings.push(MeetingListItem {
                ...
                processing_status: row.get::<String>(4)?,
                scheduled_start_at: row.get::<Option<String>>(5)?,
                actual_start_at: row.get::<Option<String>>(6)?,
                actual_end_at: row.get::<Option<String>>(7)?,
                recall_bot_id: row.get::<Option<String>>(8)?,
                latest_note_summary: row.get::<Option<String>>(9)?,
    ...
    pub async fn get_meeting_for_user(
        ...
        let mut detail = MeetingDetail {
            ...
            original_meeting_url: row.get::<String>(4)?,
            normalized_meeting_url: row.get::<String>(5)?,
            dedup_key: row.get::<String>(6)?,
            status: row.get::<String>(7)?,
            processing_status: row.get::<String>(8)?,
            scheduled_start_at: row.get::<Option<String>>(9)?,
            scheduled_end_at: row.get::<Option<String>>(10)?,
            actual_start_at: row.get::<Option<String>>(11)?,
            actual_end_at: row.get::<Option<String>>(12)?,
            deleted_at: row.get::<Option<String>>(13)?,
